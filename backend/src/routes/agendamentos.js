...
router.post("/", async (req, res, next) => {
  try {
    ...
    if (!clienteId || !tecnicoId || !data || !hora) {
      return res
        .status(400)
        .json({ error: "Selecione o cliente, técnico, data e horário. " });
    }
    ...
    if (!tecnico || tecnico.perfil !== "tecnico") {
      return res
        .status(400)
        .json({ error: 'Selecione um usuário com perfil "técnico".' });
    }
...
router.patch("/:id/cancelar", async (req, res, next) => {
  try {
    ...
    if (current.status !== "agendado") {
      return res
        .status(409)
        .json({ error: "Apenas agendamentos em aberto podem ser cancelados." });
    }
